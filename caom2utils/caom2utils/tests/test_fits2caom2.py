...
def test_augment_artifact_position_from_blueprint():
    ...
    test_blueprint.set('Chunk.position.axis.range.start.coord1.val', '128.7499990027')
    test_blueprint.set('Chunk.position.axis.range.start.coord2.pix', '513.0')
    test_blueprint.set('Chunk.position.axis.range.start.coord2.val', '-0.9999999922536')
    test_fitsparser = FitsParser(sample_file_4axes, test_blueprint, uri='test_parser')
    test_chunk = Chunk()
...
def test_augment_observation():
    ...
    test_obs_blueprint.set('Observation.telescope.geoLocationX', '-2100330.87517')
    test_obs_blueprint.set('Observation.telescope.geoLocationY', '-3694247.82445')
...
def test_update_fits_headers():
    ...
    update_blueprint(test_blueprint, test_uri, config={}, defaults={}, overrides={})
    assert test_parser.blueprint._get('Observation.type') == (['OBSTYPE'], None), 'unmodified blueprint'
    ...
    update_blueprint(test_blueprint, test_uri, config={}, defaults=test_defaults, overrides={})
    ...
    assert test_blueprint._get('Chunk.time.axis.axis.ctype') == (
        ['CTYPE3'],
        'TIME',
    ), 'default value assigned, value all upper case'
    ...
    update_blueprint(test_blueprint, test_uri, test_config, test_defaults, test_overrides)
    ...
    assert test_blueprint._get('Observation.type') == 'OBJECT', 'default value over-ridden, value all upper case'
    assert (
        test_blueprint._get('Chunk.position.axis.function.refCoord.coord1.val', 0) == '210.551666667'
    ), 'override HDU 0'
    assert (
        test_blueprint._get('Chunk.position.axis.function.refCoord.coord1.val', 1) == '210.551666667'
    ), 'override HDU 1'
    assert (
        test_blueprint._get('Chunk.position.axis.function.refCoord.coord1.val', 2) == '210.508333333'
    ), 'override HDU 2'
    assert (
        test_blueprint._get('Chunk.position.axis.function.refCoord.coord1.val', 3) == '210.898333333'
    ), 'override HDU 3'
    assert (
        test_blueprint._get('Chunk.position.axis.function.refCoord.coord1.val', 4) == '210.942083333'
    ), 'override HDU 4'
    assert (
        test_blueprint._get('Chunk.position.axis.function.refCoord.coord1.val', 5) == '0.000000000'
    ), 'override HDU 5'
    ...
    assert test_parser._headers[0]['CRVAL3'] == 56789.429806900000, 'override HDU 0'
...
TEST_OVERRIDES = {
    ...
    'artifacts': {
        ...
        'ad:CFHT/1709071g.fits.gz': {
            0: {
                'artifact.productType': 'auxiliary',
            ...
            1: {'CRPIX1': '7.00000000', 'CRPIX2': '7.00000000', 'CRVAL1': '210.551666667', 'CRVAL2': '54.526222222'},
            2: {'CRPIX1': '7.00000000', 'CRPIX2': '7.00000000', 'CRVAL1': '210.508333333', 'CRVAL2': '54.345555556'},
            3: {'CRPIX1': '7.00000000', 'CRPIX2': '7.00000000', 'CRVAL1': '210.898333333', 'CRVAL2': '54.341916667'},
            4: {'CRPIX1': '7.00000000', 'CRPIX2': '7.00000000', 'CRVAL1': '210.942083333', 'CRVAL2': '54.446805556'},
            5: {'CRPIX1': '7.00000000', 'CRPIX2': '7.00000000', 'CRVAL1': '0.000000000', 'CRVAL2': '0.000000000'},
            6: {'BITPIX': '0'},
        },
    },
}
...
def test_chunk_naxis():
    ...
    update_blueprint(test_blueprint, test_uri, config=test_config, defaults=test_defaults, overrides=test_overrides)
...
def test_file_scheme_uris():
    ...
    with patch('sys.stdout', new_callable=BytesIO) as stdout_mock, patch(
        'caom2utils.data_util.StorageInventoryClient', autospec=True
...
def _get_obs(from_xml_string):
    etree.parse = Mock(return_value=etree.ElementTree(etree.fromstring(from_xml_string.encode('ascii'))))
...
def test_augment_artifact_bounds_range_from_blueprint():
    ...
    test_blueprint.set('Chunk.position.axis.range.start.coord2.val', '-60000.0')
    test_blueprint.set('Chunk.position.axis.range.end.coord2.pix', '-824.46002')
    ...
    test_chunk.position = SpatialWCS(CoordAxis2D(Axis('RA', 'deg'), Axis('DEC', 'deg')))
    test_fitsparser._try_position_with_blueprint(test_chunk, 0)
    test_fitsparser._try_energy_with_blueprint(test_chunk, 0)
    test_fitsparser._try_time_with_blueprint(test_chunk, 0)
    test_fitsparser._try_polarization_with_blueprint(test_chunk, 0)
    ...
    assert test_chunk.energy.axis.range is not None, 'chunk.energy.axis.range should be declared'
    assert test_chunk.time.axis.range is not None, 'chunk.time.axis.range should be declared'
...
def test_visit_generic_parser():
    try:
        sys.argv = ['fits2caom2', '--local', 'fname', '--observation', 'test_collection_id', 'test_observation_id']
...
def test_get_vos_headers(vos_mock):
    ...
    try:
        caom2utils.data_util.get_local_file_headers = Mock(side_effect=_get_local_headers)
...
def test_apply_blueprint():
    ...
    hdr1['ORIGIN'] = '123'
    test_blueprint = ObsBlueprint()
    assert test_blueprint._get('Plane.provenance.producer') == (['ORIGIN'], None)
    test_blueprint.set_default('Plane.provenance.producer', 'abc')
    assert test_blueprint._get('Plane.provenance.producer') == (['ORIGIN'], 'abc')
    test_blueprint.add_attribute('Plane.provenance.producer', 'IMAGESWV')
    assert test_blueprint._get('Plane.provenance.producer') == (['IMAGESWV', 'ORIGIN'], 'abc')
...
def test_apply_blueprint_execute_external():
    ...
    test_fits_parser = FitsParser(src=sample_file_4axes, obs_blueprint=test_fits_blueprint)
    assert test_fits_parser is not None, 'expect fits construction to complete'
    assert (
        test_fits_parser._get_from_list('Observation.type', index=0) == 'fits_parser_value'
    ), 'wrong fits plan value'
...
def test_parser_construction(vos_mock, stdout_mock):
    ...
    assert (
        '<caom2:uri>vos:goliaths/abc.fits.gz</caom2:uri>' in stdout_mock.getvalue()
    ), 'Artifact URI missing from Observation'
...
def _get_node(uri, limit, force):
    node = vos.Node('abc')
    node.props = {'MD5': '5b00b00d4b06aba986c3663d09aa581f', 'length': 682560}
    return node
